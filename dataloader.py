...
# Este script prepara un dataset personalizado para entrenar un modelo LSTM que predice las coordenadas de los cuadros delimitadores de objetos en secuencias de video, de acuerdo al experimento 1.
# Recordemos que el experimento 1 consiste en usar 22 videos con todos los frames y sus GT's disponibles para train y 8 videos para test (evaluación).
...
# Configuración
otb30_videos = get_video_names()  # Lista de nombres de videos

otb30_train_videos = otb30_videos[:22]  # Primeros 30 videos para entrenamiento
otb30_test_videos = otb30_videos[22:30]  # Últimos 8 videos para test
otb30_eval_videos = otb30_videos[30:]  # Últimos 3 videos para evaluación (opcional), solo para comprobar generalización (no incluidos en el paper original)

base_path = "OTB100" # Directorio raíz del dataset
save_path_train= "data/ROLO_data_exp1/train" # Carpeta donde se guardarán los datos de entrenamiento
save_path_test = "data/ROLO_data_exp1/test" # Carpeta donde se guardarán los datos de test
save_path_eval = "data/ROLO_data_exp1/eval" # Carpeta donde se guardarán los datos de evaluación (opcional)
...
extractor = YOLOv8FeatureExtractor(model_path='yolov8s.pt') # Inicializar el extractor de características YOLOv8 (versión small)

def process_videos(video_list, base_path, save_path, extractor):
    """
    ...
    for video in video_list:
        ...
        os.makedirs(output_dir, exist_ok=True) # Crear carpeta del video si no existe
        ...
        # Obtener lista de imágenes del video y emparejar con los GTs 
        ...
        features, gts_normalized, bboxes_yolo = [], [], [] # Features, ground truths normalizados y bounding boxes predichos por YOLOv8
...
process_videos(otb30_train_videos, base_path, save_path_train, extractor)

# Procesar videos de test y evaluación
process_videos(otb30_test_videos, base_path, save_path_test, extractor)
process_videos(otb30_eval_videos, base_path, save_path_eval, extractor)
